...
const OrderCartScreen: React.FC<OrderCartScreenProps> = () => {
  ...
  return (
   <SafeAreaView style={tailwind`flex-1 p-4`}>
      ...
      <View style={tailwind`bg-white p-4 mb-4 rounded-md shadow-md flex-row items-center`}>
  ...
  <Text style={tailwind`text-lg font-bold mb-4`}>Pedido Número {order.id}</Text>
  {orderData.map((item) => (
    <View
      key={item.id}
    ...
    >
      ...
      <Text>
        {item.quantity} x {item.meal.price ? item.meal.price.toFixed(2) : 'N/A'} Kz
      </Text>
    </View>
  ))}
</View>
       ...
       <AppButton title="levar Para Client" 
        onPress={() => navigation.navigate('Delivery', {order, customerData})} color="black" 
       // style={tailwind`mt-5`}
      />
