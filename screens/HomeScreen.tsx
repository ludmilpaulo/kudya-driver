...
import { Order, Customer, Restaurant, OrderDetail, UserOrder } from '../configs/types';
...
const HomeScreen = () => {
  ...
  const [restaurantOrder, setRestaurantOrder] = useState<Restaurant | undefined>();
  ...
  useEffect(() => {
    const fetchDataFromOrderEndpoint = async () => {
      try {
        const { orders }: { orders: UserOrder[] } = await getOrder('/api/driver/orders/ready/');
